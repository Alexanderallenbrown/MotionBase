...
for i=1:length(motion_des)    % motion index
    ...
    % (maybe)
    [R_po, motors, platform_points, motorangles, R_cg] = platformposition(motion_des(i,:),angle_des(i,:), r_base, r_platform, z0_platform);
    ...
    for k = 1:6
        [F_pq] = forceplatform(m, J, R_pq(1,:), R_pq(2,:), R_pq(3,:), R_pq(4,:), R_pq(5,:), R_pq(6,:), R_cg(1,:), R_cg(2,:), R_cg(3,:), motion_des(i,:), alpha);
